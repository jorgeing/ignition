class generadorTextoCsvSimuladorPLCOmron:
	...
	def obtieneTextoTransformado(self):
		return _texto_transformado
	
	def _transformaTextoOmronASimuladorPLC():
		lineas_sin_encabezado = self._obtenerLineasSinEncabezado(self._lineasTextoGeneradoOmron)
		
		for linea in lineas_sin_encabezado:
			self._transformaLineaYConcatenaATextoTransformado(linea)
	
	def _obtenerLineasSinEncabezado(self):
		return self._lineasTextoGeneradoOmron[1:]
	...
	def _transformaYFormateaColumnas(self, columnas):
		...
		tipo_dato_mapeado_simulador = self._mapearTipoDatoOmronASimulador(tipo_dato)
